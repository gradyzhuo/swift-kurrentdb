...
extension PersistentSubscriptions.SpecifiedStream {
    ...
    public struct Read: StreamStream {
        ...
        package func send(connection: GRPCClient<Transport>, metadata: Metadata, callOptions: CallOptions, completion: @escaping @Sendable ((any Error)?) -> Void) async throws -> Responses {
            ...
            // RPC task 只能弱持有 subscription,否則會與 tracker 所存的 finish action
            // 形成保留循環(subscription → tracker → finish action → task → subscription),
            // 使 subscription 永不釋放。無法直接用 `Task { [weak subscription] in ... }`,
            // 因為巢狀的 @Sendable closure 引用該弱綁定會被 Swift 6 拒絕
            // (reference to captured var in concurrently-executing code),
            // 故改以 WeakSubscriptionRef 承載弱參照。
